...
def _load_hitad(weights: str, version: str, device: str):
    from hitad.models import HiTAD, HiTADV2, HiTADV33
    from hitad.utils.decode import decode_predictions

    factory = {"v1": HiTAD, "v2": HiTADV2, "v33": HiTADV33}
    cls = factory[version]
...
def build_predict_fn(args):
    ...
    version = {"hitad_v1": "v1", "hitad_v2": "v2",
               "hitad_v33": "v33", "hitad": "v33"}.get(mt)
    if version:
        return _load_hitad(args.weights, version, device)
...
def main():
    ...
    p.add_argument("--model-type", required=True,
                   choices=["hitad_v1", "hitad_v2", "hitad_v33",
                            "hitad", "yolo", "rtdetr", "ultralytics"])
