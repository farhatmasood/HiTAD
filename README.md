...
<p align="center">
  <img src="docs/GraphicalAbstract.png" alt="GraphicalAbstract" />
</p>

Visual representation of multi-label annotation pipeline for six spinal pathologies (above). HiTAD Framework, a foundational architecture illustrates a novel bi-staged hierarchical architecture, integrating attention-enhanced feature refinement with dual DDE and MLDR heads to mathematically disambiguate and precisely localize spatially overlapping disorders.
...
HiTAD detects **six** spinal pathologies from T1/T2-weighted mid-sagittal lumbar MRI:
...
<br/>
<sub><b>HiTAD Architecture.</b> An augmented backbone (GELAN + R-ELAN) feeds multi-scale features through an FPN neck with Area Attention (A²). The novel bi-staged detection head employs a Disjoint Disorder Extractor (DDE) for non-overlapping disorders followed by a Multi-Label Disorder Refiner (MLDR) with RoI alignment for overlapping pathologies. Soft-NMS is applied during post-processing.</sub>
</div>
...
| **HiTAD** | **74.05** | **95.27** | **74.61** | **91.49** | **91.33** | **82.62** | **84.90** |
...
| **HiTAD** | **0.8493** | **0.7834** | **0.8613** | **0.7725** | **0.7896** |
...
| **HiTAD** | **32.19** | **33.20** | **19.38** | **1.08** |

> HiTAD achieves the highest mAP with **32.19M parameters** — 33.5% fewer than YOLOv8m — while maintaining comparable training time to standard YOLO variants. RT-DETR variants exhibit 6.15× more training overhead despite comparable parameter counts, attributable to the transformer encoder's quadratic $\mathcal{O}(n^2)$ self-attention complexity.

### Clinical Validation

Comparison against expert clinicians (n=2,994 annotations across 350 images):

| Condition | HiTAD Sen | HiTAD Spe | Clinician 1 Sen | Clinician 1 Spe | Clinician 2 Sen | Clinician 2 Spe |
|:----------|:------------:|:-------------:|:---------------:|:---------------:|:---------------:|:---------------:|
...
> HiTAD achieves clinical-grade sensitivity (88.1%) and specificity (97.6%), comparable to expert clinicians, while being **180× faster**.
...
<sub><b>Qualitative Comparison.</b> HiTAD correctly identifies overlapping pathologies (SP+LDB, SS+LDB) that the second-best baseline (YOLOv26) misses or over-predicts.</sub>
</div>
...
As a post-detection step, HiTAD performs automated **Meyerding severity grading** directly from detected SP bounding boxes. The pipeline:
...
HiTAD/                     Core library
├── models/
│   ├── HiTAD.py           V1: Single-stage with AreaAttention + DFL head
│   ├── HiTAD_v2.py        V2: Two-stage (DDE + MLDR) with RoI refinement
│   └── HiTAD_v33.py       V3.3: DenseC2f + adaptive LayerNorm + AreaAttention
├── data/
...
```bash
# Clone repository
git clone https://github.com/farhatmasood/HiTAD.git
cd HiTAD
...
# Install HiTAD as editable package
pip install -e .
...
```python
from HiTAD.utils.pfirrmann import get_pfirrmann_grade
...
```bibtex
@article{masood2026HiTAD,
  title     = {A Clinically Validated Hierarchical Two-Stage Attention-Enhanced Architecture for Multi-Label Overlapping Spinal Disorder Detection},
