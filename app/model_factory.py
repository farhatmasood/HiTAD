...
Supported architectures:
    ...
    UNet++, SwinUNet, TransUNet, YOLO-Seg, HiTAD
"""
...
# ---------------------------------------------------------------------------
# HiTAD Wrapper (custom architecture or Ultralytics fallback)
# ---------------------------------------------------------------------------
try:
    from hitad.models import HiTADV33 as HiTADModel
    HITAD_MODEL_AVAILABLE = True
except ImportError:
    HITAD_MODEL_AVAILABLE = False


class HiTADWrapper(ModelWrapper):
    model_type = "detection"
    supports_gradcam = True
    ...
    def __init__(self, model_path: str):
        ...
        if not HITAD_MODEL_AVAILABLE:
            raise ImportError(
                "HiTAD model architecture not available. "
                "Install the package: pip install -e ."
            )
        checkpoint = torch.load(model_path, map_location="cpu", weights_only=False)
        if "model" not in checkpoint:
            raise ValueError("HiTAD checkpoint must contain a 'model' key.")
        self.model = HiTADModel(num_classes=NUM_CLASSES)
...
class ModelFactory:
    ...
    DISPLAY_NAMES = {
        ...
        "transunet": "TransUNet", "hitad": "HiTAD",
        "ys": "HiTAD-v2", "modelv1": "ModelV1",
    ...
    def load_model(model_name: str, model_config: Dict) -> ModelWrapper:
        ...
        wrappers = {
            ...
            "HiTADWrapper": lambda: HiTADWrapper(path),
