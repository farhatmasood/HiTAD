"""
HiTAD Training Script
=========================

Unified training pipeline for all HiTAD model variants (V1, V2, V3.3).
...
from hitad.data.dataset import (
    SpinalMRIDataset,
    collate_fn,
    get_transform,
    CLASS_NAMES,
...
from hitad.utils.metrics import compute_metrics
...
def parse_args():
    p = argparse.ArgumentParser(description="Train HiTAD")
...
def build_model(variant, num_classes, image_size, device):
    ...
    if variant == "v1":
        from hitad.models.hitad import HiTAD
        model = HiTAD(num_classes=num_classes)
    elif variant == "v2":
        from hitad.models.hitad_v2 import build_model as _build_v2
        model = _build_v2(num_classes=num_classes, image_size=image_size)
    elif variant == "v33":
        from hitad.models.hitad_v33 import HiTADV33
        model = HiTADV33(num_classes=num_classes)
    else:
...
def build_loss(variant, num_classes, image_size):
    ...
    if variant == "v2":
        from hitad.losses.two_stage_loss import HiTADLoss
        return HiTADLoss(
...
def main():
    ...
    run_name = f"hitad_{args.model}_{timestamp}"
    save_dir = os.path.join(args.save_dir, run_name)
    os.makedirs(save_dir, exist_ok=True)

    print("=" * 70)
    print(f"HiTAD {args.model.upper()} Training: {run_name}")
    ...
    if args.model == "v2":
        ...
        from hitad.losses.detection_loss import v8DetectionLoss
